...
```
## Usage
The first node started must be a Leader (see [Roles](https://github.com/zillolo/dht#Roles)).
To start a node as leader use the following:
```
node [httpHost] [serviceHost] --leader
```
Where:
* httpHost - The hostname of the HTTP API
* serviceHost - The hostname of the Leader node.

Both hostnames must include a port using the following syntax:
```
    [hostname]:[port]
```
...
For example:
```
node localhost:8888 localhost:9000 --leader
```

The HTTP API will now be available under the address `http://localhost:8888` and the Leader node will listen under the address `localhost:9000`.

All following nodes must be Followers (see [Roles](https://github.com/zillolo/dht#Roles)).
To start a node as a follower use the following:
```
node [leaderHost] [serviceHost]
```
Where:
* leaderHost - The hostname of the Leader node.
* serviceHost - The hostname of the Follower node.

## Roles
There are currently two types of roles for nodes:
* Follower
* Leader
